...
- **Tappable POI / Restaurant detail screen** (`/poi/[id]`): hero image, category kicker (with Kültür Yolu pill when relevant), serif title + Turkish subtitle, rating + XP pills, full About description, Get directions (Google Maps), Learn more (web search), placeholder "SOON" cards for Opening hours / Location & access / Menu highlights or Visitor tips, plus precise coordinates. All POI and restaurant cards on Explore and Food now navigate here. Backend endpoint: `GET /api/pois/{poi_id}`.
