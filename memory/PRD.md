...
- **Tappable POI / Restaurant detail screen** (`/poi/[id]`): hero image, category kicker (with Kültür Yolu pill when relevant), serif title + Turkish subtitle, rating + XP pills, full About description, a primary **"Check in here"** GPS button (calls `POST /api/progress/poi-check-in`, validates 150 m radius, and auto-credits **every quest** that contains this POI — the next time the user opens that quest the location is already ✓ Visited), plus Directions (Google Maps) / Learn more (web search) secondary actions, placeholder "SOON" cards for Opening hours / Location & access / Menu highlights or Visitor tips, and a precise coordinates pill.
...
- **Food tab**: now has TWO sections — "Traditional dishes" (horizontal scroller of 20 iconic Antep dishes from `gaziantep_yemekleri.json` with English descriptions) **above** the restaurant list ("Where to taste them"). Backed by a new `dishes` collection + `GET /api/cities/{id}/dishes` endpoint.
