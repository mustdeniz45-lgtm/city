...
create table if not exists public.quests (
  ...
  badge_name         text,
  trivia             jsonb,           -- { question, options[], correct_index }
  created_at         timestamptz not null default now()
);
...
create table if not exists public.profiles (
  ...
  avatar_url   text,
  device_id    text,                  -- last linked device for progress migration
  created_at   timestamptz not null default now(),
...
-- Public read for content tables; writes only via service role (RLS-bypass).
...
-- progress: read is public (leaderboard); writes only via authenticated user owning the row.
...
-- =====================================================================
-- 3. STORAGE BUCKETS  (avatars: public read, postcards: auth-only)
--    Run the bucket creation in the Supabase Dashboard → Storage UI if the
--    INSERTs below conflict on your project. Bucket policies follow.
-- =====================================================================
